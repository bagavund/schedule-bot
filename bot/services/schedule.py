...
def format_schedule(row):
    ...
    return f"""
📅 <b>{date_str} ({weekday_ru})</b>

👨‍💻 <b>Основная смена:</b> {row['Основа']}
🖥 <b>Администрирование:</b> {row['Администрирование'] if pd.notna(row['Администрирование']) else '—'}
🌙 <b>Ночная смена:</b> {row['Ночь']}
🔄 <b>Резервное дежурство:</b> {row['Резерв'] if pd.notna(row['Резерв']) else '—'}
🏖 <b>В отпуске:</b> {row['Отпуск']}
"""

def get_user_shifts(df, user_name, only_future=True):
    today = datetime.now().date()
    
    mask = ((df['Основа'] == user_name) | 
